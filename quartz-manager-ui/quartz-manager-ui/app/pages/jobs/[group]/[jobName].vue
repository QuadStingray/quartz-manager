...
import type {JobConfig, JobInformation} from "~/composables/generated/models";
...
// Route parameters
const jobGroup = route.params.group as string;
...
const fetchJobData = async () => {
  try {
    ...
    const job = await jobsApi.getJobInformation({
      jobGroup: jobGroup,
      jobName: jobName
    });
    ...
    toast.add({
      severity: 'error',
      summary: t('jobsPage.toast.jobLoadFailed'),
      detail: e.message || t('jobsPage.toast.jobLoadFailed'),
      life: 5000
    });
  } finally {
...
onMounted(async () => {
  await Promise.all([fetchJobClasses(), fetchJobData()]);
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: t('jobsPage.toast.jobUpdated'),
      detail: t('jobsPage.toast.jobUpdatedDetail', { name: jobConfig.value.name }),
      life: 3000
    });
    ...
    toast.add({
      severity: 'error',
      summary: t('jobsPage.toast.jobUpdateFailed'),
      detail: e.message || t('jobsPage.toast.jobUpdateFailed'),
      life: 5000
    });
  } finally {
    loading.value = false;
...
<template>
  <div class="card surface-0">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      />
      <h1 class="text-2xl font-bold">{{ t('jobsPage.updateJob') }}</h1>
    </div>

    <!-- Loading Spinner -->
    <div v-if="loadingJob" class="flex justify-center items-center py-8">
      <ProgressSpinner />
    </div>

    <!-- Error Message -->
    <Message v-else-if="error" severity="error" :closable="true" @close="error = null" class="mb-4">
      {{ error }}
    </Message>
