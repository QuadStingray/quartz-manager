...
object LuceneQueryParser {
  ...
  /** Parses a Lucene query string and returns a function that filters items based on field extractors.
    *
    * @param queryString
    *   The Lucene query string (e.g., "jobGroup:batch AND status:SUCCESS")
    * @param fieldExtractors
    *   Map of field names to functions that extract the field value from an item
    * @tparam T
    *   The type of items to filter
    * @return
    *   A filter function that returns true if the item matches the query
    */
  def parseAndFilter[T](queryString: Option[String], fieldExtractors: Map[String, T => Option[String]]): T => Boolean = {
    queryString match {
  ...
  private def matchesQuery[T](item: T, query: Query, fieldExtractors: Map[String, T => Option[String]]): Boolean = {
    query match {
      case bq: BooleanQuery =>
        ...
        val mustMatch = mustClauses.forall(
          clause => matchesQuery(item, clause.query(), fieldExtractors)
        )

        // At least one SHOULD clause must match (if there are any SHOULD clauses)
        val shouldMatch = shouldClauses.isEmpty || shouldClauses.exists(
          clause => matchesQuery(item, clause.query(), fieldExtractors)
        )

        // No MUST_NOT clause should match
        val mustNotMatch = mustNotClauses.forall(
          clause => !matchesQuery(item, clause.query(), fieldExtractors)
        )
