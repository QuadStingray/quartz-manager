...
trait AuthenticationService {
  ...
  protected lazy val cache: Cache[String, (UserInformation, TokenResponse)] =
    Scaffeine()
      .recordStats()
      .expireAfterWrite(ConfigService.getDuration("dev.quadstingray.quartz-manager.auth.token.duration").toScala)
      .maximumSize(500)
  ...
  lazy val securedEndpointDefinition = {
    if (ConfigService.getBoolean("dev.quadstingray.quartz-manager.auth.enabled")) {
      if (ConfigService.getBoolean("dev.quadstingray.quartz-manager.auth.basic")) {
